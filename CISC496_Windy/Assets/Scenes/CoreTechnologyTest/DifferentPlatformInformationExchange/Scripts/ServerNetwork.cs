...
class Messages{
    ...
    public static byte[] CreateSetAvatarPositionRotationMessage(
        ...
        //byte[] m = new byte[14];
        // ...
        m[0] = Messages.setAvatarPositionRotation;
        m[1] = avatarId;
        m[2] = movementState;
        int count = 3;
        AddIntToByteArray((short)Mathf.RoundToInt(x * 100f), ref m, ref count);
        AddIntToByteArray((short)Mathf.RoundToInt(z * 100f), ref m, ref count);
        AddIntToByteArray((short)Mathf.RoundToInt(r * 100f), ref m, ref count);
    ...
    static float ExtractFloat(byte[] message, int startPos) {
		Debug.Assert(startPos >= 0 && startPos+2 <= message.Length);////////////////////////////////////Debug.Assert(startPos >= 0 && startPos+2 < message.Length);
    ...
    public static void UnmarshallPositionRotationMessage(
		...
		Debug.Assert(message.Length >= 9); ////////////////////////////////////////////////////////////Debug.Assert(message.Length >= 14);
        ...
        // ...
        x = ExtractFloat(message, 3) / 100f;
        z = ExtractFloat(message, 5) / 100f;
        r = ExtractFloat(message, 7) / 100f;
    }
...
public class ServerNetworkScript : MonoBehaviour {
	...
	void UpdateUI() {
		...
		GameObject.Find("FirstConnectionId").GetComponent<Text>().text = "";
		GameObject.Find("FirstAvatarName").GetComponent<Text>().text = "";
		GameObject.Find("SecondConnectionId").GetComponent<Text>().text = "";
		GameObject.Find("SecondAvatarName").GetComponent<Text>().text = "";
	...
	void AddNewClient(int recHostId, int connectionId, int channelId) {
		...
		NetworkTransport.Send(recHostId, connectionId, channelId, Messages.CreateSetPlayerIdMessage(connectionId), 2, out error);
	...
	/// <summary>
	/// When a client window shuts down, a disconnection occurs.
	/// This function is to remove the client from the server, and update its avatar if necessary.
	/// </summary>
	/// <param name="connectionId">connection id of the client who left</param>
	void RemoveClient(int connectionId) {

		Debug.Assert(connections.ContainsKey(connectionId));
		byte avatar = connections[connectionId];
		if (connections.Remove(connectionId)) {
			Logger.LogFormat("Player {0} has been successfully removed", connectionId);
		} else {
			Logger.LogFormat("Server failed in removing player {0}", connectionId);
		}

		if (connections.Count > 0 && avatar == Messages.flora)
		{
			int connectionIdForOtherClient = new List<int>(connections.Keys)[0];
			Logger.LogFormat("Setting avatar for player {0} to {1}", connectionIdForOtherClient, Messages.flora);
			connections[connectionIdForOtherClient] = Messages.flora;
			byte error;
			NetworkTransport.Send(hostId, connectionIdForOtherClient, controlChannelId, Messages.CreateUseAvatarMessage(Messages.flora), 2, out error);
			Messages.LogNetworkError(error);
		}
	}
	...
	void ForwardMovementMessage(int senderConnectionId, byte[] message, int messageLength) {
		...
		foreach (var client in connections) {
			if (client.Key != senderConnectionId) {
				NetworkTransport.Send(hostId, client.Key, dataChannelId, message, messageLength, out error);
				Messages.LogNetworkError(error);
			}
		}
	}
		...
		while(messagesAvailable) {
			...
			case NetworkEventType.DisconnectEvent:
				Logger.Log(string.Format("Disconnection received from host {0}, connection {1}, channel {2}",
					...
					RemoveClient(connectionId); // The function is below AddNewClient.
				break;
			}
		}
	}
